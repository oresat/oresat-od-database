...
def add_publish_data(od: canopen.ObjectDictionary, config: OdConfig, core=True):
    ...
    for i in config.publish:
        ...
        com_rec = canopen.objectdictionary.Record(f'Publish data {num} communication parameters',
                                                  com_index)
        map_rec = canopen.objectdictionary.Record(f'Publish data {num} mapping parameters',
                                                  map_index)
...
def add_all_subscribe_data(master_node_od: canopen.ObjectDictionary,
                           node_od: canopen.ObjectDictionary):

    if 0x1800 not in node_od and 0x1801 not in node_od \
            and 0x1802 not in node_od and 0x1803 not in node_od:
        return
    ...
    for i in range(4):
        ...
        com_rec = canopen.objectdictionary.Record(
            f'Subscribe {node_name} data communication parameters',
            com_index)
        ...
        map_rec = canopen.objectdictionary.Record(f'Subscribe {node_name} data mapping parameters',
                                                  map_index)
